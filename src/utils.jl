...
function generate_mpVI(prob::DyNEP, T_hor::Int64)
    ...
    Q = [BlockDiagonal([kron(I(T_hor - 1), prob.Q[i]), prob.P[i]]) for i in 1:N]
    ...
    # Define H 
    H = BlockDiagonal(R) + BlockArray([Γi[i]' * Q[i] * Γi[j] for i in 1:N, j in 1:N])

    # Define F (maps from x0 to VI mapping)
    F = vcat([Γi[i]' * Q[i] * Θ for i in 1:N]...)

    # define f (affine part of the VI mapping)
    f = zeros(sum(prob.nu) * prob.T_hor)

    ## Constraints
    # D_shar = row_stack([I ⊗ Du_i ; (I ⊗ Dx)*Γi ] )
    D_shar = hcat([[kron(I(T_hor), prob.C_u_i[i]); kron(I(T_hor), prob.Cx) * Γi[i]] for i in 1:N]...)
    # Append local constraints 
    D = [D_shar; prob.C_loc] # WRONG!

    # Define E (maps from x0 to constraints)
    E = [zeros(T_hor * prob.m_u, prob.nx); kron(I(T_hor), prob.Cx) * Θ; zeros(sum(prob.m_loc) * T_hor)]

    # Affine part of the constraints
    d = [kron(ones(T_hor), prob.b_u); kron(ones(T_hor), prob.b_x)] # MISSING LOCAL CONSTRAINTS!

    # VI(H*x + F*x0 + f, D*x <= E*x0 + d)
    mpVI = MPVI(H, F, f, D, E, d)
end

function generate_prediction_model(A::Matrix{Float64}, Bi::Vector{Matrix{Float64}}, T_hor::Int)
    n_x = size(A, 1)
