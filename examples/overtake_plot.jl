...
function plot_overtake(method::Symbol)
    road_width = 10.0
    road_length = 1000.0
    vehicle_width = 2.
    ...
    if method == :DR
        ...
        pv_loaded = CSV.read("pos_vel_DR.csv", DataFrame)
    end
    if method == :ADMM
        println("Plotting results from ADMM...")
        pv_loaded = CSV.read("pos_vel_ADMM.csv", DataFrame)
    end
    if method == :DGSQP
        println("Plotting results from DGSQP...")
        pv_loaded = CSV.read("pos_vel_DGSQP.csv", DataFrame)
    end
    if method == :Explicit
        println("Plotting results from Explicit...")
        pv_loaded = CSV.read("pos_vel_explicit.csv", DataFrame)
    end
    p1 = pv_loaded.p1
    v1 = pv_loaded.v1
    l1 = pv_loaded.l1
    ...
    # Save GIF
    if method == :DR
        gif(anim, "vehicle_animation_DR.gif", fps=10)
        mp4(anim, "vehicle_animation_DR.mp4", fps=10)
    end
    if method == :ADMM
        gif(anim, "vehicle_animation_ADMM.gif", fps=10)
        mp4(anim, "vehicle_animation_ADMM.mp4", fps=10)
    end
    if method == :DGSQP
        gif(anim, "vehicle_animation_DGSQP.gif", fps=10)
        mp4(anim, "vehicle_animation_DGSQP.mp4", fps=10)
    end
    if method == :Explicit
        gif(anim, "vehicle_animation_Explicit.gif", fps=10)
        mp4(anim, "vehicle_animation_Explicit.mp4", fps=10)
    end
