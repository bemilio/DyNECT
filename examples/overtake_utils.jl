...
# Unicycle model
function unicycle!(dx::AbstractVector, x::AbstractVector, u::AbstractVector, t::Float64) # t is needed for compatibility with DynamicalSystems.jl even though not used
    # x = (pos, vel, lat_pos)
    # u = (acc, steer_angle)
    dx[1] = x[2] * cos(u[2])
    dx[2] = u[1]
...
function state_to_posvel(x::Vector{Float64})
...
function posvel_to_state(posvel::Vector{Float64})
    x = zeros(5)
    x[1] = posvel[2]
    x[2] = posvel[3]
...
function choose_controller(posvel::Vector{Float64}, vref::Vector{Float64}, dref::Vector{Float64}, lref::Vector{Float64}, tol::Float64, current_case::Case)
...
### Plotting utilities ###
function compute_normal_tangent(x::Vector{Float64}, y::Vector{Float64})
    dx = diff(x)
    dy = diff(y)
...
function drawRoad!(plt, x::AbstractVector{Float64}, y::AbstractVector{Float64}, normal_vecs::AbstractMatrix{Float64}, width::Real, color::Symbol)
...
function drawRectangle!(plt, x::Real, y::Real, angle::Real, height::Real, width::Real, color::Symbol)
