...
for (i, (param, param_label)) in enumerate(params_list)
    ...
    params = params[keep]
    vals = vals[keep]
    yticks = [10.0^i for i in -7:2:4]
    push!(all_plots, boxplot(params, # Reduce horizontal distance between boxes
        vals,
        xlabel=latexstring(param_label),
        ylabel=(raw"$\mathrm{Time (s)}$"),
        xguidefontsize=18,
        yguidefontsize=18,
        legendfontsize=12,
        tickfontsize=12,
        legend=:topright,
        label="Explicit Build Time",   # Custom legend name
        color=metric_fields[1][:color],
        yaxis=:log10,
        ylim=(1e-7, 1e4),
        yticks=yticks,
        bar_width=0.3,         # narrower boxes
    ))

    ##### 2. Evaluation time: explicit vs implicit side-by-side
    ...
    for p in all_param_vals
        ...
        for v in vals_exp[findall(==(p), params_exp)]
            push!(eval_times, v)
            push!(eval_param, p)
            push!(eval_group, "Explicit eval. time")
        end
        # Implicit
    ...
    yticks = ([10.0^i for i in -7:2:4], fill("", 12)) # Define the y ticks, but write an empty string in the associated y axis
...
# combined = plot(reordered_plots..., layout=layout, size=(1200, 2400), left_margin=[20Plots.mm, 0Plots.mm])
left_margin = [20Plots.mm -5Plots.mm 20Plots.mm -5Plots.mm 20Plots.mm -5Plots.mm 20Plots.mm -5Plots.mm]
combined = plot(reordered_plots..., layout=layout, size=(1200, 2400), margin=0Plots.mm, left_margin=left_margin)
display(combined)
